...
async function run(action, data={}, message='Updated.') {
  ...
  try {
    ...
    if(!['generate','save','export'].includes(action)) {
      const saved=await call({action:'save'});
      try { localStorage.setItem('coastal-layout',saved.content); localStorage.setItem('coastal-strains',JSON.stringify(state.strains)); localStorage.setItem('coastal-rooms',JSON.stringify(state.rooms)); }
      catch { status('Updated, but browser storage is unavailable. Save a layout file to keep your work.',true); return; }
    }
    status(message);
...
(async()=>{try{state=await call({action:'state'});try{for(const room of JSON.parse(localStorage.getItem('coastal-rooms')||'[]'))await call({action:'design',spec:room});for(const [name,abbreviation] of Object.entries(JSON.parse(localStorage.getItem('coastal-strains')||'{}')))if(!state.strains[name])await call({action:'strain',name,abbreviation});const saved=localStorage.getItem('coastal-layout');state=await call(saved?{action:'restore',content:saved}:{action:'state'});status('Ready.');}catch(error){state=await call({action:'state'});status(`Could not restore browser autosave: ${error.message}. Open a saved layout or start a new one.`,true);}render();$('#application').disabled=false;}catch(error){status(error.message,true);}})();
